...
def probability_of_good_flankscore(rows: list[dict[str, str]]) -> None:
    groups = {
    ...
    for row in rows:
        aoa2 = float(row["aoa2"])
        flankscore = float(row["flankscore"])
        ...
        if flankscore < GOOD_FLANKSCORE_THRESHOLD:
    ...
    for group_name, stats in groups.items():
        ...
        print(f"  good flankscores: {good}")
        print(f"  probability of a good flankscore: {probability:.4f}")


if __name__ == "__main__":
    probability_of_good_flankscore(load_rows(DATA_FILE))
